...
v2.0 adds:
  - ingest_workbook() — primary entry point for Excel/XLSX files
  - Integrates pricing_sheet_classifier (zone-aware, handles mixed tabs)
  ...
  - Backward compatible: parse_and_normalize and run() still work
"""
import io
import logging
...
class PricingAgent(BaseAgent):
    ...
    Wraps existing pricing_analyzer + pricing_parser + classifier/mapper/validator.
    """
    ...
    def __init__(self, base_currency: str = "USD"):
        ...
        tools = [
            Tool(
                ...
                description="Ingest an Excel workbook — detect pricing sheets, map columns, validate",
    ...
    def _ingest_workbook(
        ...
        """
        Full pipeline for Excel workbook ingestion:
          1. Parse workbook sheets
          2. Classify all sheets — detect pricing, technical, mixed
          3. Pick best pricing sheet (or pricing zone in a mixed tab)
          4. Map columns to canonical schema
          5. Validate — formula checks, ACV math, confidence tier
        """
        try:
            import openpyxl
        ...
        # Extract all sheets as row lists
        sheet_data: Dict[str, List[List[Any]]] = {}
        for sheet_name in wb.sheetnames:
            ws = wb[sheet_name]
            sheet_data[sheet_name] = [list(row) for row in ws.iter_rows(values_only=True)]
        ...
        # Augment column map from RFP template if provided
        ...
        # Get pricing zones (handles mixed/combined tabs)
        pricing_zones = get_all_pricing_zones(best_sheet, cl)
        zones_arg = pricing_zones if cl.get("sheet_type") == "mixed" else None
        ...
        return {
            ...
            "sheet_type": cl.get("sheet_type", "pricing"),
            "schema": canonical,
            ...
            "sheet_classifications": {k: {
                ...
                "sheet_type": v.get("sheet_type", "unknown"),
            } for k, v in classifications.items()},
        }

    # ── Text-based ingestion (existing behavior) ─────────────────────────────
